---
title: Judge0 CE - Code Execution API
emoji: 🚀
colorFrom: indigo
colorTo: purple
sdk: docker
pinned: false
license: mit
---

# Judge0 CE - Code Execution API

This is a self-hosted instance of Judge0 CE (Community Edition) on Hugging Face Spaces.

## Features

- Execute code in 60+ programming languages
- RESTful API
- Free to use
- Perfect for coding platforms, educational tools, and developer utilities

## API Usage

### Submit Code for Execution

```bash
curl -X POST "https://xabir-judge0.hf.space/submissions?base64_encoded=false&wait=true" \
  -H "Content-Type: application/json" \
  -d '{
    "source_code": "print(\"Hello World\")",
    "language_id": 71
  }'
```

### Response

```json
{
  "stdout": "Hello World\n",
  "time": "0.001",
  "memory": 3456,
  "stderr": null,
  "token": "abc123",
  "compile_output": null,
  "message": null,
  "status": {
    "id": 3,
    "description": "Accepted"
  },
  "language_id": 71
}
```

## Language IDs

| ID | Language |
|----|----------|
| 45 | Assembly (NASM 2.14.02) |
| 46 | Bash (5.0.0) |
| 47 | Basic (FBC 1.07.1) |
| 75 | C (Clang 7.0.1) |
| 76 | C++ (Clang 7.0.1) |
| 48 | C (GCC 7.4.0) |
| 52 | C++ (GCC 7.4.0) |
| 49 | C (GCC 8.3.0) |
| 53 | C++ (GCC 8.3.0) |
| 50 | C (GCC 9.2.0) |
| 54 | C++ (GCC 9.2.0) |
| 86 | Clojure (1.10.1) |
| 51 | C# (Mono 6.6.0.161) |
| 77 | COBOL (GnuCOBOL 2.2) |
| 55 | Common Lisp (SBCL 2.0.0) |
| 56 | D (DMD 2.089.1) |
| 57 | Elixir (1.9.4) |
| 58 | Erlang (OTP 22.2) |
| 44 | Executable |
| 87 | F# (.NET Core SDK 3.1.202) |
| 59 | Fortran (GFortran 9.2.0) |
| 60 | Go (1.13.5) |
| 88 | Groovy (3.0.3) |
| 61 | Haskell (GHC 8.8.1) |
| 62 | Java (OpenJDK 13.0.1) |
| 63 | JavaScript (Node.js 12.14.0) |
| 78 | Kotlin (1.3.70) |
| 64 | Lua (5.3.5) |
| 89 | Multi-file program |
| 79 | Objective-C (Clang 7.0.1) |
| 65 | OCaml (4.09.0) |
| 66 | Octave (5.1.0) |
| 67 | Pascal (FPC 3.0.4) |
| 85 | Perl (5.28.1) |
| 68 | PHP (7.4.1) |
| 43 | Plain Text |
| 69 | Prolog (GNU Prolog 1.4.5) |
| 70 | Python (2.7.17) |
| 71 | Python (3.8.1) |
| 80 | R (4.0.0) |
| 72 | Ruby (2.7.0) |
| 73 | Rust (1.40.0) |
| 81 | Scala (2.13.2) |
| 82 | SQL (SQLite 3.27.2) |
| 83 | Swift (5.2.3) |
| 74 | TypeScript (3.7.4) |
| 84 | Visual Basic.Net (vbnc 0.0.0.5943) |

## Rate Limits

- No rate limiting (free tier)
- May have cold start delay (~30 seconds) after inactivity

## Credits

- Original Judge0: https://github.com/judge0/judge0
- Hosted on Hugging Face Spaces: https://huggingface.co/spaces
